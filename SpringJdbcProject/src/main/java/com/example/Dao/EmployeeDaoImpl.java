...
import com.exmple.bean.EmployeeDeptBean;
import com.exmple.bean.EmployeeDesignationBean;
...
//=========================================================================================
	...
	public FectchEmployeeDetailsDto fetchEmployeeNameByEmpIdUsingMap(FetchEmployeeRequest fetchEmployeeRequest) {
		...
		FectchEmployeeDetailsDto fetchEmployeeDeatilsDto = new FectchEmployeeDetailsDto();
		...
		fetchEmployeeDeatilsDto = new FectchEmployeeDetailsDto();
...
//==============================================================
	...
	public List<FectchEmployeeDetailsDto> fetchAllEmployeesUsingList(FetchEmployeeRequest fetchEmployeeRequest) {
		...
		List<FectchEmployeeDetailsDto> fetchEmployeeList = new ArrayList<>();
		...
		List<Map<String, Object>> rows = namedParameterJdbcTemplate.queryForList(query.toString(),
				new BeanPropertySqlParameterSource(fetchEmployeeRequest));
		FectchEmployeeDetailsDto fetchEmployeeDeatilsDto = null;
...
//========================================================
	...
	public List<FectchEmployeeDetailsDto> fetchEmployeeNameByEmpIdUsingQuery(FetchEmployeeRequest fetchEmployeeRequest) {
		...
		return namedParameterJdbcTemplate.query(queryForFetchEmployeeNmByEmpIdUsingQuery,
				new BeanPropertySqlParameterSource(fetchEmployeeRequest),
				new BeanPropertyRowMapper<FectchEmployeeDetailsDto>(FectchEmployeeDetailsDto.class));

	}

//======================================================
	@Override
	public List<FectchEmployeeDetailsDto> fetchAllEmployeeDetails(FetchEmployeeRequest fetchEmployeeRequest) {
		...
		List<FectchEmployeeDetailsDto> fetchEmployeeDeatilsDtoList = new ArrayList<>();
	...
	public List<FectchEmployeeDetailsDto> fetchEmployeeNameByEmpId3(FetchEmployeeRequest fetchEmployeeRequest) {
		...
		String queryForFetchEmployeeNmByEmpId = "select er.emp_id,first_nm,last_nm,middle_nm,mobile_no,email_id,address,state_nm,country_nm,pincode,city_nm,qualification_nm,designation_nm ,emp_dept_id,edp.dept_id,dept_nm,emp_designation_id,designation_id,edp.isactive,eds.isactive from employee_ref er left join employee_dept_ref edp on er.emp_id=edp.emp_id join deprtment_ref dr ON edp.dept_id=dr.dept_id left join employee_designation_ref eds on er.emp_id=eds.emp_id where   edp.isactive=true order by er.emp_id";

		List<Map<String, Object>> rows = namedParameterJdbcTemplate.queryForList(queryForFetchEmployeeNmByEmpId,
				new BeanPropertySqlParameterSource(fetchEmployeeRequest));
		List<FectchEmployeeDetailsDto> empDetailsList = null;
		...
		if (CollectionUtils.isNotEmpty(rows)) {
			...
			for (Map<String, Object> row : rows) {
				...
				if (set.add((Long) row.get("emp_id"))) {// 1 2

					if (fetchEmployeeDeatilsDto != null) {
						if (CollectionUtils.isNotEmpty(empDesignationList)) {
							fetchEmployeeDeatilsDto.setDesignationList(empDesignationList);
						}

						if (CollectionUtils.isNotEmpty(empDeptList)) {
							fetchEmployeeDeatilsDto.setDepartmentList(empDeptList);
						}
